	...
	return array(
		...
		array(
			...
			'content' => '<ul class="diako-legal-list">'
				...
				. '<li><strong>' . esc_html__( 'محصول:', 'diako' ) . '</strong> ' . esc_html__( 'کالاهای فیزیکی مراقبت پوست، آرایشی، مراقبت مو/بدن و ابزار زیبایی (و در صورت ارائه، کارت هدیه دیجیتال).', 'diako' ) . '</li>'
		...
		array(
			...
			'highlight' => true,
			'content'   => '<p class="diako-legal-emphasis">' . esc_html__( 'فروشگاه انزی تحت هیچ شرایطی استرداد وجه (Refund) انجام نمی‌دهد. تمامی فروش‌ها نهایی (All Sales Are Final) محسوب می‌شوند.', 'diako' ) . '</p>'
				. '<ul class="diako-legal-list">'
				. '<li>' . esc_html__( 'به دلایل بهداشتی، محصولات مراقبت پوست و آرایشی پس از باز شدن پلمپ یا استفاده، قابل بازگشت نیستند.', 'diako' ) . '</li>'
				. '<li>' . esc_html__( 'بازگشت کالا به دلیل «انصراف از خرید»، «پشیمانی»، «عدم رضایت از نتیجه روی پوست» یا «تغییر نظر» پذیرفته نمی‌شود.', 'diako' ) . '</li>'
		...
		array(
			'id'      => 'digital',
			'title'   => __( '۶. نکات بهداشتی و حساسیت', 'diako' ),
			'content' => '<ul class="diako-legal-list">'
				. '<li>' . esc_html__( 'مشتری مسئول است پیش از خرید، ترکیبات محصول و تناسب آن با نوع پوست خود را بررسی کند.', 'diako' ) . '</li>'
				. '<li>' . esc_html__( 'در صورت سابقه حساسیت پوستی، پیش از استفاده تست پچ انجام دهید؛ فروشگاه مسئول واکنش‌های آلرژیک یا حساسیت فردی نیست.', 'diako' ) . '</li>'
				. '<li>' . esc_html__( 'تاریخ انقضا و شرایط نگهداری روی بسته‌بندی یا صفحه محصول درج می‌شود؛ پس از تحویل، نگهداری صحیح بر عهده مشتری است.', 'diako' ) . '</li>'
				. '</ul>',
		),
		array(
			'id'      => 'warranty',
		...
		array(
			...
			'content' => '<ul class="diako-legal-list">'
				. '<li>' . esc_html__( 'فروشگاه در حدود قوانین جاری، مسئول خسارات غیرمستقیم، نتایج زیبایی مورد انتظار شخصی، یا مشکلات ناشی از استفاده نادرست از کالا پس از تحویل نیست.', 'diako' ) . '</li>'
				. '<li>' . esc_html__( 'اطلاعات آموزشی مجله و صفحات محصول جایگزین مشاوره پزشکی یا پوستی تخصصی نیستند.', 'diako' ) . '</li>'
				. '</ul>',
		),
		array(
			'id'      => 'privacy',
...
function diako_render_terms_page() {
	...
	<div class="diako-page diako-legal-page">
		...
		<div class="diako-legal-page__notice">
			...
			<p>
				<?php esc_html_e( 'فروشگاه انزی تحت هیچ شرایطی استرداد وجه انجام نمی‌دهد. با تکمیل خرید، این سیاست را می‌پذیرید.', 'diako' ); ?>
			</p>
		</div>
